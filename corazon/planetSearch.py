...
def identifyTces(time, flux, bls_durs_hrs=[1, 2, 4, 8, 12], minSnr=3, fracRemain=0.5, maxTces=10, minP=None, maxP=None):
    """
    ...
    while keepLooking:
        ...
        transit_mask = bls.transit_mask(t, bls_results[0], bls_results[3] * 1.1, bls_results[1])
        # plt.figure()
        ...
        # Intendend bitwise operation?
        #  if not, use "and" and not "&"
        if (len(t)/len(time) > fracRemain) & (bls_results[4] >= minSnr) & (counter <= maxTces):
